...
- Auth/Anthropic: preserve user-selected auth mode for `anthropic:default` during external CLI sync by refreshing Claude CLI credentials only when the existing profile is already OAuth, preventing `api_key`/`token` profiles from being auto-converted.
