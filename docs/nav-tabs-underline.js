(() => {
  ...
  const setupNavTabsObserver = (tabs) => {
    ...
    if (lastX !== null && lastWidth !== null) {
      // eslint-disable-next-line restrict-template-expressions -- narrowed to non-null above; TS doesn't track through the if-guard in plain JS
      navTabs.style.setProperty("--nav-tab-underline-x", `${lastX}px`);
      // eslint-disable-next-line restrict-template-expressions -- narrowed to non-null above
